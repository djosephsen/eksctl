...
func AppendAuthenticator(c *clientcmdapi.Config, spec *api.ClusterConfig, command string) {
	c.AuthInfos[c.CurrentContext].Exec = &clientcmdapi.ExecConfig{
		...
		Command:    command,
		Args:       []string{"token", "-i", spec.ID.Name},
		/*
...
func MaybeDeleteConfig(lc *api.ClusterLocator) {
	p := AutoPath(lc.Name)
	...
	if autoConfExists {
		if err = isValidConfig(p, lc.Name); err != nil {
	...
	if !deleteClusterInfo(config, lc) {
		return
	}
...
func deleteClusterInfo(existing *clientcmdapi.Config, lc *api.ClusterLocator) bool {
	isChanged := false
	clusterName := lc.String()
